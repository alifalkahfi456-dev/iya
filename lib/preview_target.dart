...
const String _baseUrl = 'http://lightsecret.duckdns.top:10005';
...
class _C {
  ...
  static const Color borderLit = Color(0xFF4A4A4A);
  static const Color blueDark = Color(0xFF4A4A4A);
  static const Color blueMid = Color(0xFF6B6B6B);
  static const Color blueLight = Color(0xFF808080);
  static const Color green = Color(0xFF4CAF50);
  static const Color greenDim = Color(0xFF388E3C);
  ...
  static const LinearGradient btnGrad = LinearGradient(
    colors: [Color(0xFF6B6B6B), Color(0xFF808080), Color(0xFF9E9E9E)],
...
class _BugSenderPageState extends State<BugSenderPage>
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (data["valid"] == true) {
          ...
          // Handle format { private: [...], global: [...] }
          if (conns is Map) {
            var private = conns["private"] ?? [];
            if (private is! List) private = [];
            conns = private.where((s) =>
              s is Map && s["sessionName"] != null &&
              s["sessionName"].toString().isNotEmpty
            ).toList();
          } else if (conns is! List) {
            conns = [];
          }
          setState(() {
            senderList = conns;
          ...
          setState(() => errorMessage = data["message"] ?? "Gagal mengambil data");
        }
      } else {
        setState(() => errorMessage = "Server error: ${res.statusCode}");
      }
    } catch (e) {
      setState(() => errorMessage = "Koneksi gagal");
    } finally {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        if (data["valid"] == true) {
          ...
          _toast(data['message'] ?? "Gagal generate pairing code", error: true);
        }
      } else {
        _toast("Server error: ${res.statusCode}", error: true);
      }
    } catch (_) {
      _toast("Koneksi gagal", error: true);
    } finally {
  ...
  Future<void> _deleteSender(String senderId) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        if (data["valid"] == true) {
          _toast("Sender berhasil dihapus");
          _fetchSenders();
        } else {
          _toast(data["message"] ?? "Gagal", error: true);
        }
      } else {
        _toast("Server error: ${res.statusCode}", error: true);
      }
  ...
  Widget _buildSenderCard(Map<String, dynamic> sender, int index) {
    final name = sender['sessionName'] ??
                sender['number'] ??
                sender['id'] ??
                'Unknown Sender';
    final isPublic = sender['isPublic'] == true;
  ...
  Widget _buildBody() {
    if (isLoading && senderList.isEmpty && !_hasLoaded) return const Center(child: _DotsLoader());
    if (errorMessage != null && senderList.isEmpty && !_hasLoaded) return _ErrorState(message: errorMessage!, onRetry: _fetchSenders);
    if (senderList.isEmpty) return _EmptyState(onAdd: _showAddSenderDialog);
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (ctx, i) => _buildSenderCard(Map<String, dynamic>.from(senderList[i]), i),
...
class _SenderCardState extends State<_SenderCard> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: _C.blueDark.withOpacity(0.07), blurRadius: 24, offset: const Offset(0, 8))],
      ),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(20),
        child: Column(
                    ...
                    children: [
                      Container(
                        ...
                        child: Stack(
                          children: [
                            const Center(
                              child: Icon(FontAwesomeIcons.whatsapp, color: _C.blueLight, size: 24),
                            ),
                            Positioned(
                              right: 5, bottom: 5,
                              child: AnimatedBuilder(
                                ...
                                builder: (_, __) => Container(
                                  ...
                                  decoration: BoxDecoration(
                                    shape: BoxShape.circle,
                                    color: widget.isConnected ? _C.green.withOpacity(_dot.value) : _C.red,
                                    boxShadow: widget.isConnected ? [BoxShadow(
                                      color: _C.green.withOpacity(_dot.value * 0.6), blurRadius: 6,
                                    )] : null,
                                  ),
                                ),
                              ),
                              ...
                              if (widget.isPublic)
                                Container(
                                  ...
                                  child: const Text('PUBLIC',
                                      style: TextStyle(color: _C.blueLight, fontSize: 8, fontWeight: FontWeight.w700)),
                                ),
                            ]),
                          ],
                        ),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: widget.isConnected ? _C.green.withOpacity(0.1) : _C.red.withOpacity(0.1),
                          borderRadius: BorderRadius.circular(8),
                          border: Border.all(
                            color: widget.isConnected ? _C.green.withOpacity(0.3) : _C.red.withOpacity(0.3),
                          ),
                        ),
                        child: Text(
...
class _CardBtn extends StatefulWidget {
  ...
  const _CardBtn({required this.label, required this.icon, required this.onTap, this.isDestructive = false});
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        height: 42,
        decoration: BoxDecoration(
          color: _pressed ? color.withOpacity(0.15) : color.withOpacity(0.06),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: _pressed ? color.withOpacity(0.5) : color.withOpacity(0.2)),
        ),
        child: AnimatedScale(
          ...
          child: Row(
            ...
            children: [
              ...
              const SizedBox(width: 6),
              Text(widget.label, style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.w600)),
            ],
          ),
        ),
      ),
...
class _PairingDialog extends StatefulWidget {
  ...
  const _PairingDialog({required this.number, required this.code, required this.onClose});

  @override
  State<_PairingDialog> createState() => _PairingDialogState();
}

class _PairingDialogState extends State<_PairingDialog> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _glow = Tween<double>(begin: 0.3, end: 0.9).animate(CurvedAnimation(parent: _glowCtrl, curve: Curves.easeInOut));
  }
          ...
          AnimatedBuilder(
            ...
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: _C.blueDark.withOpacity(0.12),
                boxShadow: [BoxShadow(color: _C.blueMid.withOpacity(_glow.value * 0.4), blurRadius: 30)],
                border: Border.all(color: _C.blueMid.withOpacity(_glow.value * 0.5)),
              ),
              child: const Icon(Icons.phonelink_lock_rounded, color: _C.blueLight, size: 28),
            ),
          ),
          const SizedBox(height: 16),
          const Text('Kode Pairing', style: TextStyle(color: _C.text, fontSize: 20, fontWeight: FontWeight.w800)),
          const SizedBox(height: 6),
          Text('Nomor: ${widget.number}', style: const TextStyle(color: _C.textSub, fontSize: 13)),
          const SizedBox(height: 24),
          AnimatedBuilder(
            animation: _glow,
            builder: (_, __) => Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: _C.blueMid.withOpacity(_glow.value * 0.6), width: 1.5),
                boxShadow: [BoxShadow(color: _C.blueMid.withOpacity(_glow.value * 0.25), blurRadius: 20)],
              ),
              child: Text(
                widget.code,
                textAlign: TextAlign.center,
                style: TextStyle(
                  ...
                  fontSize: 36, fontWeight: FontWeight.w900, letterSpacing: 10, fontFamily: 'monospace',
                ),
              ),
            ),
          ),
          const SizedBox(height: 12),
          _CopyBtn(code: widget.code, onCopied: () => setState(() => _copied = true), copied: _copied),
...
class _CopyBtn extends StatelessWidget {
  ...
  final bool copied;
  const _CopyBtn({required this.code, required this.onCopied, required this.copied});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      height: 44,
      child: _PressableInk(
        onTap: copied ? null : () async { await Clipboard.setData(ClipboardData(text: code)); onCopied(); },
        borderRadius: 12,
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 250),
          decoration: BoxDecoration(
            color: copied ? _C.green.withOpacity(0.12) : _C.surface,
            borderRadius: BorderRadius.circular(12),
            border: Border.all(color: copied ? _C.green.withOpacity(0.4) : _C.border),
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              AnimatedSwitcher(
                ...
                child: Icon(copied ? Icons.check_rounded : Icons.copy_rounded,
                    key: ValueKey(copied), color: copied ? _C.green : _C.textSub, size: 16),
              ),
              const SizedBox(width: 8),
              AnimatedSwitcher(
                duration: const Duration(milliseconds: 200),
                child: Text(copied ? 'Disalin!' : 'Salin Kode',
                    key: ValueKey(copied),
                    style: TextStyle(color: copied ? _C.green : _C.textSub, fontWeight: FontWeight.w600, fontSize: 13)),
              ),
            ],
          ),
        ),
...
class _StatStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.wifi_tethering_rounded, color: _C.blueLight, size: 18),
          const SizedBox(width: 10),
          Text('$total sender terdaftar', style: const TextStyle(color: _C.text, fontSize: 13, fontWeight: FontWeight.w600)),
          const Spacer(),
          Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            AnimatedBuilder(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  shape: BoxShape.circle,
                  border: Border.all(color: _C.borderLit),
                  boxShadow: [BoxShadow(color: _C.blueDark.withOpacity(0.2), blurRadius: 30)],
                ),
                child: const Icon(FontAwesomeIcons.whatsapp, color: _C.blueLight, size: 38),
              ),
            ),
            const SizedBox(height: 28),
            const Text('Belum Ada Sender', style: TextStyle(color: _C.text, fontSize: 20, fontWeight: FontWeight.w700)),
            const SizedBox(height: 10),
            const Text(
            ...
            _GradBtn(label: 'Tambah Sender', icon: Icons.add_rounded, onTap: widget.onAdd),
          ],
        ),
      ),
    );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.wifi_off_rounded, color: _C.red, size: 30),
            ),
            const SizedBox(height: 24),
            const Text('Koneksi Gagal', style: TextStyle(color: _C.text, fontSize: 20, fontWeight: FontWeight.w700)),
            const SizedBox(height: 10),
            Text(message, textAlign: TextAlign.center, style: const TextStyle(color: _C.textSub, fontSize: 13, height: 1.5)),
            const SizedBox(height: 32),
            _GradBtn(label: 'Coba Lagi', icon: Icons.refresh_rounded, onTap: onRetry),
          ],
        ),
      ),
    );
      ...
      child: AnimatedScale(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(13),
            boxShadow: _down ? [] : [BoxShadow(color: _C.blueMid.withOpacity(0.3), blurRadius: 16, offset: const Offset(0, 4))],
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            ...
            children: [
              ...
              Text(widget.label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 14, letterSpacing: 0.3)),
            ],
          ),
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: Text(widget.label, style: const TextStyle(color: _C.textSub, fontWeight: FontWeight.w600, fontSize: 14)),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        floatingLabelStyle: const TextStyle(color: _C.blueMid, fontSize: 12),
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: _C.blueMid, width: 1.5)),
      ),
    );
  }
}
...
class _AppBarBtnState extends State<_AppBarBtn> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(_AppBarBtn old) {
    ...
    if (widget.spinning && !old.spinning) { _spinCtrl.repeat(); }
    else if (!widget.spinning) { _spinCtrl.stop(); _spinCtrl.reset(); }
  }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: AnimatedBuilder(
          animation: _spinCtrl,
          builder: (_, child) => Transform.rotate(angle: _spinCtrl.value * math.pi * 2, child: child),
          child: Icon(widget.icon, color: widget.onTap == null ? _C.textDim : _C.textSub, size: 18),
        ),
      ),
    );
  }
...
class _PressableInk extends StatelessWidget {
  ...
  const _PressableInk({required this.child, this.onTap, this.borderRadius = 12});
...
class _BgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      final paint = Paint()
        ..shader = RadialGradient(
          colors: [_C.blueDark.withOpacity(0.07 - i * 0.015), Colors.transparent],
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height * 0.4), vigPaint);
  }
